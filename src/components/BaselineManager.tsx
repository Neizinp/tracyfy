...
export function BaselineManager({ baselines, onCreateBaseline, onViewBaseline }: BaselineManagerProps) {
                    ...
                    <div className="space-y-3">
                        {sortedBaselines.map((baseline, index) => (
                            <div
                                ...
                                className={`p-4 rounded-lg border cursor-pointer transition ${selectedId === baseline.id
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2 text-xs text-gray-400 mt-3">
                                    ...
                                    {baseline.addedArtifacts && baseline.addedArtifacts.length > 0 && (
                                        <>
                                            <span className="mx-2">•</span>
                                            <span className="text-green-400">+{baseline.addedArtifacts.length} added</span>
                                        </>
                                    )}
                                    {baseline.removedArtifacts && baseline.removedArtifacts.length > 0 && (
                                        <>
                                            <span className="mx-2">•</span>
                                            <span className="text-red-400">-{baseline.removedArtifacts.length} removed</span>
                                        </>
                                    )}
                                </div>
