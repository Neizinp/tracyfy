                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                    <thead>
                        <tr style={{ background: 'var(--color-bg-secondary)', borderBottom: '1px solid var(--color-border)' }}>
                            ...
                            {visibleColumns.rationale && <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600, color: 'var(--color-text-secondary)', minWidth: '200px' }}>Rationale</th>}
                            {visibleColumns.author && <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600, color: 'var(--color-text-secondary)', width: '120px' }}>Author</th>}
                            {visibleColumns.verification && <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600, color: 'var(--color-text-secondary)', width: '150px' }}>Verification</th>}
                            {visibleColumns.priority && <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600, color: 'var(--color-text-secondary)', width: '100px' }}>Priority</th>}
                            {visibleColumns.status && <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600, color: 'var(--color-text-secondary)', width: '100px' }}>Status</th>}
                            {visibleColumns.comments && <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600, color: 'var(--color-text-secondary)', minWidth: '200px' }}>Comments</th>}
                            {visibleColumns.created && <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600, color: 'var(--color-text-secondary)', width: '120px' }}>Created</th>}
                            {visibleColumns.approved && <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600, color: 'var(--color-text-secondary)', width: '120px' }}>Approved</th>}
                        </tr>
                    </thead>
                    <tbody>
                        {requirements.length === 0 ? (
                            <tr>
                                <td colSpan={getVisibleColumnCount()} style={{ padding: '32px', textAlign: 'center', color: 'var(--color-text-muted)' }}>
                            ...
                            requirements.map(req => (
                                ...
                                >
                                    ...
                                    {showProjectColumn && (
                                        <td style={{ padding: '12px 16px', verticalAlign: 'top' }}>
                                            <div style={{
                                                ...
                                                {getProjectNames(req.id).split(', ').map((name, i) => (
                                                    name && (
                                                        <span key={i} style={{
                                                            fontSize: '0.75rem',
                                                            padding: '2px 6px',
                                    ...
                                    {visibleColumns.priority && (
                                        <td style={{ padding: '12px 16px', verticalAlign: 'top' }}>
                                            <span style={{
                                                ...
                                                fontSize: '0.75rem',
                                                fontWeight: 500,
                                                background: req.priority === 'high' ? 'rgba(239, 68, 68, 0.1)' :
                                                    req.priority === 'medium' ? 'rgba(245, 158, 11, 0.1)' :
                                                        'rgba(16, 185, 129, 0.1)',
                                                color: req.priority === 'high' ? '#ef4444' :
                                                    req.priority === 'medium' ? '#f59e0b' :
                                                        '#10b981'
                                            }}>
                                        ...
                                        <td style={{ padding: '12px 16px', verticalAlign: 'top' }}>
                                            <span style={{
                                                ...
                                                background: req.status === 'approved' ? 'rgba(16, 185, 129, 0.1)' :
                                                    req.status === 'draft' ? 'rgba(107, 114, 128, 0.1)' :
                                                        'rgba(59, 130, 246, 0.1)',
                                                color: req.status === 'approved' ? '#10b981' :
                                                    req.status === 'draft' ? '#6b7280' :
                                                        '#3b82f6'
                                            }}>
                                    ...
                                    {visibleColumns.created && (
                                        <td style={{ padding: '12px 16px', verticalAlign: 'top', color: 'var(--color-text-secondary)', whiteSpace: 'nowrap' }}>
