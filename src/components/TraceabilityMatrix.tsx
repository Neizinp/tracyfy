...
export const TraceabilityMatrix: React.FC<TraceabilityMatrixProps> = ({ requirements, links }) => {
    ...
    const getCellContent = (rowReq: Requirement, colReq: Requirement): { text: string; color: string; tooltip: string } | null => {
        ...
        <div style={{
            ...
            <div style={{ marginBottom: 'var(--spacing-md)' }}>
                ...
                <p style={{ fontSize: '0.875rem', color: 'var(--color-text-secondary)' }}>
                    Shows relationships between requirements. ↓P=Parent, ↑C=Child, ↔=Relates, →=Depends, ⚠=Conflicts
                </p>
            </div>
                ...
                <table style={{
                    ...
                    <tbody>
                        {requirements.map((rowReq, rowIndex) => (
                            <tr key={rowReq.id} style={{
                                backgroundColor: rowIndex % 2 === 0 ? 'transparent' : 'var(--color-bg-hover)'
                            }}>
                                <td style={{
                                    position: 'sticky',
                                    left: 0,
                                    backgroundColor: rowIndex % 2 === 0 ? 'var(--color-bg-card)' : 'var(--color-bg-hover)',
                                    padding: '8px',
                                    border: '1px solid var(--color-border)',
                                    fontWeight: 600,
                                ...
                                {requirements.map(colReq => {
                                    ...
                                    return (
                                        <td key={colReq.id} style={{
                                            padding: '8px',
                                            border: '1px solid var(--color-border)',
                                            textAlign: 'center',
                                            backgroundColor: cellContent ? cellContent.color : 'transparent',
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', gap: 'var(--spacing-md)', marginTop: '4px', flexWrap: 'wrap' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <span style={{ padding: '2px 6px', backgroundColor: 'rgba(34, 197, 94, 0.2)', borderRadius: '4px' }}>↓ P</span>
                        Parent
                    </span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <span style={{ padding: '2px 6px', backgroundColor: 'rgba(59, 130, 246, 0.2)', borderRadius: '4px' }}>↑ C</span>
                        Child
                    </span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <span style={{ padding: '2px 6px', backgroundColor: 'rgba(168, 85, 247, 0.2)', borderRadius: '4px' }}>↔</span>
                        Relates To
                    </span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <span style={{ padding: '2px 6px', backgroundColor: 'rgba(251, 146, 60, 0.2)', borderRadius: '4px' }}>→</span>
                        Depends On
                    </span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <span style={{ padding: '2px 6px', backgroundColor: 'rgba(239, 68, 68, 0.2)', borderRadius: '4px' }}>⚠</span>
                        Conflicts With
                    </span>
                </div>
            </div>
