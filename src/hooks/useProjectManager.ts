...
export const useProjectManager = () => {
    ...
    const initializeApp = async () => {
      ...
      try {
        ...
        const {
          ...
          currentProjectId: initialCurrentId,
          globalState,
        } = loadProjects();
  ...
  const handleResetToDemo = async () => {
    if (
      !confirm(
        'This will replace all current projects with the Demo Project containing sample data and reload the page. Continue?'
      )
    ) {
      return;
    }
  ...
  return {
    ...
    setHasInitializedProjects: (value: boolean) => {
      hasInitializedProjects.current = value;
    },
  };
};
