...
export async function exportProjectToJSON(
    ...
    // Filter artifacts
    const requirements = globalState.requirements.filter(r =>
        projectRequirementIds.includes(r.id) && !r.isDeleted
    );
    const useCases = globalState.useCases.filter(u =>
        projectUseCaseIds.includes(u.id) && !u.isDeleted
    );
    const testCases = globalState.testCases.filter(t =>
        projectTestCaseIds.includes(t.id) && !t.isDeleted
    );
    const information = globalState.information.filter(i =>
    ...
    const links = globalState.links.filter(l =>
        validIds.has(l.sourceId) || validIds.has(l.targetId)
    );
