...
describe('excelExportUtils', () => {
    ...
    it('should create separate sheets for each artifact type', async () => {
        ...
        expect(mockBookAppendSheet).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'Requirements');
        expect(mockBookAppendSheet).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'Use Cases');
        expect(mockBookAppendSheet).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'Test Cases');
        expect(mockBookAppendSheet).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'Information');
        expect(mockBookAppendSheet).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'Traceability Matrix');
    });
    ...
    it('should exclude deleted artifacts', async () => {
        ...
        const reqCall = calls.find(call => Array.isArray(call[0]) && call[0].length > 0 && call[0][0].ID === 'r1');
    ...
    it('should fallback to writeFile if showSaveFilePicker is not available', async () => {
        ...
        expect(mockWriteFile).toHaveBeenCalledWith(expect.anything(), 'Test_Project_Baseline_1_0-export.xlsx');
    });
});
